...
@section('main')
    ...
    <div class="profile-detail mb-5">
        <ul class="generic-list-item generic-list-item-flash">
            ...
            <li><span class="profile-name">نوع شخصیت: </span><span class="profile-desc">{{(Auth::user()->personality_type)}} </span>
                @if(!Auth::user()->personality_type)
                    <span style="color: red;">  کاربر گرامی لطفا نوع شخصیت خود را در تنظیمات پروفایل کامل کنید.</span>
                @endif
            </li>
            <li><span class="profile-name">ملیت: </span><span class="profile-desc">{{(Auth::user()->nationality)}} </span>
                @if(!Auth::user()->nationality)
                    <span style="color: red;">  کاربر گرامی لطفا نام و نام خانوادگی خود را در تنظیمات پروفایل کامل کنید.</span>
                @endif
            </li>

            <li><span class="profile-name">نام و نام خانوادگی: </span><span class="profile-desc">{{(Auth::user()->name)}} </span>
                @if(!Auth::user()->name)
                    <span style="color: red;">  کاربر گرامی لطفا نام و نام خانوادگی خود را در تنظیمات پروفایل کامل کنید.</span>
                @endif
            </li>
            <li><span class="profile-name"> جنسیت: </span>
            ...
            <li><span class="profile-name"> نام پدر: </span>
            ...
            <li><span class="profile-name">  تاریخ تولد: </span>
            ...
            <li><span class="profile-name"> کد ملی: </span>
            ...
            <li><span class="profile-name">    شماره شناسنامه: </span>
                <span  class="profile-desc">{{(Auth::user()->birth_certificate)}} </span>
                @if(!Auth::user()->birth_certificate)
                    <span style="color: red;">کاربر گرامی لطفا شماره شناسنامه خود را در تنظیمات پروفایل کامل کنید.</span>
                @endif
            </li>
            <li><span class="profile-name">  وضعیت تاهل : </span>
            ...
            <li><span class="profile-name"> مدرک تحصیلی: </span>
                <span class="profile-desc">@foreach(\App\Models\Profile\Educations::whereId(Auth::user()->education_id)->pluck('title') as $title){{$title}}@endforeach</span>
                @if(!Auth::user()->education_id)
                    <span style="color: red;"> کاربر گرامی لطفا مدرک تحصیلی خود را در تنظیمات پروفایل کامل کنید.</span>
                @endif
            </li>
            <li><span class="profile-name">  شغل: </span>
                <span class="profile-desc">{{Auth::user()->user_job}}</span>
                @if(!Auth::user()->user_job)
                    <span style="color: red;"> کاربر گرامی لطفا شغل خود را در تنظیمات پروفایل وارد کنید.</span>
                @endif
            </li>
            <li><span class="profile-name"> محل اشتغال: </span>
                <span class="profile-desc">@foreach(\App\Models\Profile\State::whereId(Auth::user()->place_id)->pluck('title') as $title){{$title}}@endforeach</span>
                @if(!Auth::user()->place_id)
                    <span style="color: red;"> کاربر گرامی لطفا محل اشتغال خود را در تنظیمات پروفایل کامل کنید.</span>
                @endif
            </li>
            @if(Auth::user()->type_id == 4)
                <li><span class="profile-name"> عنوان شغل: </span>
                    <span class="profile-desc">@foreach(\App\Models\Profile\Job_title::whereId(Auth::user()->job_title)->pluck('title') as $title){{$title}}@endforeach</span>
                    @if(!Auth::user()->job_title)
                        <span style="color: red;"> کاربر گرامی لطفا عنوان شغلی خود را در تنظیمات پروفایل کامل کنید.</span>
                    @endif
                </li>
                <li><span class="profile-name"> شماره پروانه: </span>
                    <span class="profile-desc">{{(Auth::user()->folder_id)}}</span>
                    @if(!Auth::user()->folder_id)
                        <span style="color: red;"> کاربر گرامی لطفا شماره پروانه خود را در تنظیمات پروفایل کامل کنید.</span>
                    @endif
                </li>
                <li><span class="profile-name"> پایه پروانه: </span>
                    <span class="profile-desc">@foreach(\App\Models\Profile\Base_certificate::whereId(Auth::user()->folder_base)->pluck('title') as $title){{$title}}@endforeach    </span>
                    @if(!Auth::user()->folder_base)
                        <span style="color: red;"> کاربر گرامی لطفا پایه پروانه خود را در تنظیمات پروفایل کامل کنید.</span>
                    @endif
                </li>
                <li><span class="profile-name">اعتبار پروانه: </span>
                    <span  class="profile-desc">{{(Auth::user()->folder_validity)}}</span>
                    @if(!Auth::user()->folder_validity)
                        <span style="color: red;"> کاربر گرامی لطفا نوع پروانه خود را در تنظیمات پروفایل کامل کنید.</span>
                    @endif
                </li>
            @endif
            <li><span class="profile-name">تلفن ثابت: </span>
            ...
            <li><span class="profile-name"> استان: </span>
                <span class="profile-desc">@foreach(\App\Models\Profile\State::whereId(Auth::user()->state_id)->pluck('title') as $title){{$title}}@endforeach </span>
            ...
            <li><span class="profile-name"> شهرستان: </span>
                <span class="profile-desc">@foreach(\App\Models\Profile\City::whereId(Auth::user()->city_id)->pluck('title') as $title){{$title}}@endforeach</span>
            ...
            <li>
                ...
                <span class="profile-desc">@foreach(\App\Models\TypeUser::whereId(Auth::user()->type_id)->pluck('title_fa') as $title){{$title}}@endforeach </span>
            ...
            <li><span class="profile-name"> شماره تلفن: </span>
                ...
                @if(Auth::user()->phone_verify == 0)
                    <span style="color: red;"> کاربر گرامی لطفا با کلیک بر روی شماره موبایل خود آن را تایید نمایید.</span>
                @endif
            </li>
        </ul>
    </div>
